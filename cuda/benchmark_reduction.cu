...
BENCHMARK_DEFINE_F(ReductionFixture, BM_FindMaxBaseline)
(benchmark::State& state) {
  for (auto _ : state) {
    ...
    thrust::device_ptr<float> device_ptr(device_array);
    CUDA_CALL(cudaEventRecord(start));
    float max_device = thrust::reduce(device_ptr, device_ptr + n, 0.0f,
    ...
    //                      cudaMemcpyDeviceToHost));
    EXPECT_FLOAT_EQ(max_host, max_device);
...
BENCHMARK_DEFINE_F(ReductionFixture, BM_FindMaxCPUBaseline)
(benchmark::State& state) {
  for (auto _ : state) {
    // Generate random number
    state.PauseTiming();
    ...
    state.ResumeTiming();
    float max_host = HostFindMax(array, n);
    benchmark::DoNotOptimize(max_host);
  }
  state.SetBytesProcessed(static_cast<int64_t>(state.iterations()) *
                          state.range(0) * 4);
}
...
(benchmark::State& state) {
  for (auto _ : state) {
    ...
    // CUDA_CALL(cudaEventRecord(start));
    // FindMax0<<<4, 1024, 1024 * sizeof(float)>>>(device_array, n,
    // device_array); CUDA_CALL(cudaEventRecord(stop)); FindMax0<<<1, 4, 1024 *
    // sizeof(float)>>>(device_array, n, device_array);
...
(benchmark::State& state) {
  for (auto _ : state) {
    ...
    // CUDA_CALL(cudaEventRecord(start));
    // FindMax1<<<4, 1024, 1024 * sizeof(float)>>>(device_array, n,
    // device_array); CUDA_CALL(cudaEventRecord(stop)); FindMax1<<<1, 4, 1024 *
    // sizeof(float)>>>(device_array, n, device_array);
...
BENCHMARK_REGISTER_F(ReductionFixture, BM_FindMaxBaseline)
    ->RangeMultiplier(2)
    ->Range(1 << 12, 1 << 28)
    ->UseManualTime()
    ->Unit(benchmark::kMillisecond);

BENCHMARK_REGISTER_F(ReductionFixture, BM_FindMaxCPUBaseline)
    ->RangeMultiplier(2)
    ->Range(1 << 12, 1 << 28)
    ->Unit(benchmark::kMillisecond);
